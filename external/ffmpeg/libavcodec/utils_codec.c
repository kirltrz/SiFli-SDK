...
const char av_codec_ffversion[] = "FFmpeg version 3.0";
...
static int update_frame_pool(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
    {
        ...
        if (avctx->codec_id == AV_CODEC_ID_EZIP565 || avctx->codec_id == AV_CODEC_ID_EZIP888)
        {
            if (frame->pkt_size <= frame->last_pkt_size)
                return 0;
            frame->last_pkt_size = frame->pkt_size;
        }
        else if (pool->format == frame->format &&
